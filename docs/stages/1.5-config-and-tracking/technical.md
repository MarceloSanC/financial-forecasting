...
### 2026-06-29 — [deviation] Auditoria de testes — C3 (run_id desconhecido) era divergência silenciosa fake↔real — Claude (autonomous)
**Contexto:** A auditoria de testes (mutação mental + paridade fake↔real, I3) revelou que o caso C3 não tinha teste no contrato e que fake e real DIVERGIAM: a doc/concept §6 C3 afirmava "`start_run(run_id=...)` com id inexistente abre um run novo (não é erro)" — verdadeiro só no fake; o `mlflow` real levanta `MlflowException: Run ... not found` (o id de um run é atribuído pelo backend, não pelo chamador). Um fake MAIS permissivo que o real dá falsa confiança aos testes upstream (Stage 5.4 `TrainTft` consome este port).
**Gap fechado:** Alinhei o contrato à realidade do backend escolhido: (a) o adapter captura `MlflowException` em `start_run` e re-levanta `LookupError` (mesma postura da padronização C2→`RuntimeError`); (b) o fake passou a recusar `run_id` fornecido mas não registrado com `LookupError` (só `run_id is None` cria run novo); (c) corrigi a narrativa C3 no port, no fake e implicitamente no concept §6 (o id não é escolhido pelo chamador). Teste adicionado `test_start_run_with_unknown_run_id_raises` (parametrizado fake+real). Mutação coberta: remover o `try/except` do adapter ou o `raise LookupError` do fake quebra agora.

### 2026-06-29 — [deviation] Auditoria de testes — reabertura nomeada e `log_metrics` sem `step` sem cobertura observável — Claude (autonomous)
**Contexto:** Dois caminhos de comportamento de contrato não eram exercitados: (1) o ramo de reabertura por `run_id` COM `run_name` (atualizar nome sem duplicar, I2) — `test_idempotent_resume_by_run_id` reabre sem nome; (2) `log_metrics` no happy-path com `step` default (None) — o default só era atingido no caso de erro sem run ativo. Mutação removendo o ramo de rename ou o default `step=None` não falhava nenhum teste.
**Gap fechado:** Adicionados ao contract test (parametrizados fake+real) `test_resume_with_run_name_keeps_same_run` (resume por id + nome devolve o mesmo id; log subsequente prova run ativo) e `test_log_metrics_without_step_on_active_run` (métrica aceita com passo implícito). Ambos passam idênticos em fake e real, reforçando a paridade I3.
